...
const SOURCE_REL =
  'source/layers/vr/frontier/model-village/model-village-character-appearance-h3w-expressive-lighting.holo';
const POLICY_REL =
  'source/proofs/model-village-character-appearance-h3w-expressive-lighting-policy.hsplus';
const SEED_REL =
  'source/proofs/model-village-character-appearance-h3w-expressive-lighting-seed.hs';
const MANIFEST_REL =
  'source/layers/vr/frontier/model-village/model-village-character-appearance-h3w-expressive-lighting-manifest.holo';
const CHECKER_REL = 'scripts/check-hololand-model-village-character-appearance-h3w.mjs';
const TEST_REL =
  'scripts/__tests__/hololand-model-village-character-appearance-h3w.test.mjs';
const REPORT_REL =
  'docs/reports/model-village-character-appearance-h3w-expressive-lighting-2026-07-29.md';
const HERO_REL =
  'docs/assets/model-village/model-village-character-appearance-h3w-expressive-lighting-2026-07-29.png';
const EVIDENCE_REL =
  'docs/assets/model-village/model-village-character-appearance-h3w-expressive-lighting-2026-07-29.json';
const OUTPUT_REL = '.tmp/hololand/model-village/character-appearance-h3w';
const EXPECTED_COMMIT = '09fe4773d58122927eabb9787c9fc2fcb4e486ba';
...
async function loadH3WToolchain(holoScriptRoot, outputDir) {
  ...
  const parserPath = path.join(outputDir, 'h3w-native-parsers.mjs');
  const compilerPath = path.join(outputDir, 'h3w-character-webgpu-compiler.mjs');
  await esbuild.build({
    stdin: {
      contents: [
      ...
      sourcefile: 'h3w-native-parsers.entry.ts',
      loader: 'ts',
    },
    outfile: parserPath,
    bundle: true,
  ...
  await esbuild.build({
    ...
    plugins: [
      {
        name: 'h3w-character-render-source-resolution',
        setup(build) {
          build.onResolve({ filter: /^@holoscript\/engine$/ }, () => ({
            path: 'h3w-character-render-source',
            namespace: 'h3w-character-render',
          }));
          build.onLoad({ filter: /.*/, namespace: 'h3w-character-render' }, () => ({
            contents:
...
export async function parseH3WStack(
  root = ROOT,
  holoScriptRoot = DEFAULT_HOLOSCRIPT_ROOT,
  outputDir = path.join(root, OUTPUT_REL)
) {
  const stack = await loadH3WToolchain(holoScriptRoot, outputDir);
  ...
  if (manifest && (!manifest.success || manifest.errors.length)) {
    stack.esbuild.stop?.();
    throw new Error(`H3W manifest parse failed: ${JSON.stringify(manifest.errors)}`);
  }
...
export function validateH3WContract(
  stack,
  root = ROOT,
  holoScriptRoot = DEFAULT_HOLOSCRIPT_ROOT
) {
  ...
  const { metadata, state, objects } = stack.h3wContract;
  expect(
    metadata.milestone === 'MV_CHARACTER_APPEARANCE_H3W_EXPRESSIVE_LIGHTING',
    'milestone drifted'
  );
  ...
  for (const [index, object] of (stack.h3wSource.ast.objects || []).entries()) {
  ...
  return {
    ...
    setup(build) {
      build.onResolve({ filter: /\.wgsl/ }, (args) => ({
        ...
        namespace: 'h3w-wgsl-raw',
      }));
      build.onLoad({ filter: /.*/, namespace: 'h3w-wgsl-raw' }, (args) => ({
...
async function buildNodeHostRuntime(stack, holoScriptRoot, outputDir) {
  mkdirSync(outputDir, { recursive: true });
  const outfile = path.join(outputDir, 'h3w-host-runtime.mjs');
  await stack.esbuild.build({
    stdin: {
      ...
      `,
      resolveDir: holoScriptRoot,
      sourcefile: 'h3w-host-runtime.entry.ts',
      loader: 'ts',
    },
    outfile,
    bundle: true,
  ...
  const result = await stack.esbuild.build({
    stdin: {
      contents: `
        ...
        window.__H3W_RUNTIME__ = { renderCharacter };
      `,
      resolveDir: holoScriptRoot,
      sourcefile: 'h3w-browser-runtime.entry.ts',
      loader: 'ts',
    },
    bundle: true,
    format: 'iife',
...
async function buildCompiledPayload(stack, plan, holoScriptRoot, outputDir) {
  ...
  for (const resident of plan) {
    const compile = () =>
      new stack.toolchain.CharacterWebGPUCompiler({
        ...
        entityId: `model-village-h3w-${resident.modelFamilyId}`,
        lodLevel: 0,
      }).compile(stack.h3wSource.ast);
    ...
    const built = hostRuntime.buildCharacterHostFromComposition(stack.h3wSource.ast, {
      objectId: resident.objectId,
      entityId: `model-village-h3w-${resident.modelFamilyId}-host`,
      lodLevel: 0,
    });
    assert(
    ...
    assert(
      built.jointDeformation?.profile === 'expressive-neck-scapular-volume-v3' &&
      ...
      `${resident.displayLabel} expressive neck/scapular receipt drifted`
    );
    assert(
    ...
    assert(
      built.expression?.schemaVersion === 'holoscript.native-facial-morph.v2' &&
  ...
  return {
    payload: {
      schema: 'hololand.model-village.character-appearance-h3w-browser-payload.v1',
...
function browserApplication() {
  return String.raw`
(() => {
  const payload = window.__H3W_PAYLOAD__;
  const runtime = window.__H3W_RUNTIME__;
  const state = {
    schema: 'hololand.model-village.character-appearance-h3w-browser-state.v1',
    ready: false,
    status: 'booting',
    gpu: null,
    residents: [],
  ...
  async function boot() {
    ...
    for (const resident of payload.residents) {
      ...
      metricsNode.textContent =
        Math.round(metrics.luminanceRange) + ' luma / ' +
        environmentDifference.changedPixelCount.toLocaleString() + ' relit pixels';
    ...
    document.querySelector('[data-status]').textContent =
      'Native Chrome WebGPU expressive-lighting witness complete';
    device.destroy?.();
  }
    ...
    (name, index) => `
      <article class="resident resident-${index + 1}">
        ...
        <div class="copy">
          ...
          <h2>${name}</h2>
          <p>V7 scapular asymmetry / authored expression</p>
...
<title>Stormglass H3W Expressive Lighting</title>
<style>
...
<main class="shell">
  <header>
    <div>
      <div class="kicker">HoloScript H3W / native expressive lighting</div>
      <h1>A village that looks<br>back at you.</h1>
    </div>
    <p class="lede">Four source-authored residents, rendered through the HoloScript
      character compiler and browser WebGPU path. Independent scapular motion,
      conversational neck pose, asymmetric eyelids and brows, and a warm-key /
      cool-fill / copper-rim environment replace the neutral identity-card read.</p>
  </header>
  <section class="grid">${residentCards()}</section>
  <footer>
    <span data-status>Acquiring browser GPU...</span>
    <span>Three-point material response / source expression / no RTX timing claim</span>
  </footer>
</main>
<script>window.__H3W_PAYLOAD__=${safeInlineJson(payload)};</script>
...
async function waitForReady(client, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const ready = await evaluate(client, 'window.__H3W__?.ready === true', 5_000).catch(
      () => false
    );
    if (ready) return;
    await delay(250);
  }
  throw new Error('Timed out waiting for the H3W browser witness');
}
...
async function runBrowserWitness({ browserPath, html, outputDir }) {
  ...
  const profileDir = path.join(os.tmpdir(), `hololand-h3w-${process.pid}-${Date.now()}`);
  ...
  try {
    ...
    const state = await evaluate(client, 'JSON.parse(JSON.stringify(window.__H3W__))', 60_000);
    ...
    const screenshotPath = path.join(outputDir, 'h3w-expressive-lighting.png');
...
export async function runCharacterAppearanceH3W(options = {}) {
  ...
  const stack = await parseH3WStack(root, holoScriptRoot, outputDir);
  try {
    const validation = validateH3WContract(stack, root, holoScriptRoot);
    ...
    const receipt = {
      schema: 'hololand.model-village.character-appearance-h3w-witness.v1',
      ...
      boundaries: {
        ...
        sourcePoseApplied: true,
        expressiveNeckScapularVolumeReceipted: true,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  runCharacterAppearanceH3W(parseArgs())
    .then(({ receipt }) => {
      if (process.argv.includes('--json')) console.log(JSON.stringify(receipt, null, 2));
      else {
        console.log(
          `PASS H3W expressive lighting: ${receipt.compilerAdmission.residentCount} residents; ` +
